...
df = df.dropna(subset=['RELH', 'VPRES', 'WSPD', 'PREC'])

# Use estimated parameters from Variance Gamma Process (manually set)
mu_hat = -0.005
sigma_hat = 0.12
nu_hat = 0.08
...
# Compute Variance Gamma process variance
df['vg_variance'] = np.nan
df['vg_variance'].iloc[0] = df['returns'].var()
np.random.seed(42)

for t in range(1, len(df)):
    vt = max(df['vg_variance'].iloc[t-1], 1e-6)  # Ensure non-negative variance
    epsilon = np.random.normal(0, 1)
    gamma_process = np.random.gamma(shape=nu_hat, scale=1.0)  # Gamma-distributed jumps
    
    # Simulate Variance Gamma variance
    vt_new = vt + mu_hat + sigma_hat * epsilon + gamma_process
    df.loc[df.index[t], 'vg_variance'] = max(vt_new, 1e-6)  # Ensure non-negative variance

# Setup the columns for training and target features
X = df[['VPRES', 'WSPD', 'PREC', 'vg_variance']].values
y = df['RELH'].values.reshape(-1, 1)
